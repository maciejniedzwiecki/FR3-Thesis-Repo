...
# Filtrowanie plików
all_files = [f for f in files if f.startswith("att_mean_modified") and all(x not in f for x in ["REFERENCYJNY", "MIEDZ", "ZESPOLONA"])]
excluded_files = [f for f in files if f.startswith("att_mean_modified") and any(x in f for x in ["MIEDZ", "ZESPOLONA"])]
...
draw_plot(all_files, "Average material losses (without Copper sheet & IRR Glass)", "att_all.png", y_min, y_max)
draw_plot(excluded_files, "Average material losses (Copper sheet & IRR Glass)", "att_excluded.png", excluded_y_min, excluded_y_max)
